...
class CardMatrix:
    ...
    def get_card_indices(self):
        return self.card2index

    def get_category_indices(self):
        return self.category2index
    ...
    def eval_cards(self, spendings: dict, ids: list) -> float:
        ...
        net_rewards = torch.dot(
            torch.concat((fees, credits, max_categories_rewards), dim=-1),
            spending_tensor,
        )
        ...
        return {"value": net_rewards, "choices": how_to_spend}
...
if __name__ == "__main__":
    ...
    def max_comboing(card_matrix, spending, whitelist=None, blacklist=None, k=5):
        ...
        card_combinations_filtered = [
            c for c in card_combinations if all([x in c for x in whitelist])
        ]
    ...
    combo, results = max_comboing(
        A, spending, whitelist=whitelist, blacklist=blacklist, k=k
    )
    val = results["value"]
